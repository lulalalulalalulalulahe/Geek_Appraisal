...
int main()
{
    ...
    for(int i = 0; i < 27; i++)
    {
        ...
        {
            arr_small[count2++] = arr_big[i];	//记录出现过的字母的个数并赋给arr_small[count2]  /*补充注释（说明作用）*/
        }
    }
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)   //筛选出非叶结点  /*补充判断模块注释（说明作用）*/
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1)  //筛选出具有最小权值的结点  /*补充判断模块注释（说明作用）*/
        {
            min = huffman[i][3];
            ii[0] = i;
        }
    ...
    
    {
		...
		if(huffman[i][3]>=min&&huffman[i][3]< min2&& huffman[i][0]== -1)
		{
			min2 = huffman[i][3];
			ii[1] = i;
		}
...
{
    int length = 0;
    if(huffman[i][0] == -1)    // 如果结点i没有父节点   /*补充判断模块注释*/
    {
        return 0;
    }
